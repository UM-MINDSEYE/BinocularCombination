...
# --- Start Screen ---
start_instructions = (
    "You will be presented with black crosshairs in front of some wider horizontal stripes. "
    "Your task is to move the horizontal line of the crosshairs into the center of the wide white bar in the middle. "
    "You do this by using the up/down arrow keys.\n\n"
    "When the narrow black horizontal line is aligned in the center of the white stripe, "
    "you press the space bar, to continue to the next trial.\n\n"
    "If needed, you can first re-adjust the mirrors during the calibration screen, "
    "which you continue from by also pressing the space bar."
)

start_text_left = visual.TextStim(win, text=start_instructions, color='white', pos=(-nonius_offset_x, nonius_offset_y), wrapWidth=300) # Reduced wrapWidth
start_text_right = visual.TextStim(win, text=start_instructions, color='white', pos=(nonius_offset_x, -nonius_offset_y), wrapWidth=300) # Reduced wrapWidth

start_text_left.draw()
start_text_right.draw()
win.flip()
...
for t_idx, trial in enumerate(trial_list):
    ...
    if (t_idx + 1) == 61: # After the 60th trial (index 59), for the 61st iteration
        ...
        break_text_content = "Break time! Press 'K' to continue."
        break_text_left = visual.TextStim(win, text=break_text_content, color='white', pos=(-nonius_offset_x, nonius_offset_y))
        break_text_right = visual.TextStim(win, text=break_text_content, color='white', pos=(nonius_offset_x, -nonius_offset_y))
        break_text_left.draw()
        break_text_right.draw()
        win.flip()
        event.waitKeys(keyList=['k'])
        event.clearEvents() # Clear events again after 'K' is pressed
